...
class BoyListAdapter(
    private val onSkuSelected: (DeliveryBoyResponse) -> Unit
) : RecyclerView.Adapter<BoyListAdapter.SkuViewHolder>() {

    private var skuList: List<DeliveryBoyResponse> = emptyList()

    fun submitList(newList: List<DeliveryBoyResponse>) {
        skuList = newList
    ...
    inner class SkuViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(sku: DeliveryBoyResponse) {

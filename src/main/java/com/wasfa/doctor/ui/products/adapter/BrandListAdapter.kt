...
class BrandListAdapter(
    private val onSkuSelected: (Brands) -> Unit
) : RecyclerView.Adapter<BrandListAdapter.SkuViewHolder>() {

    private var skuList: List<Brands> = emptyList()

    fun submitList(newList: List<Brands>) {
        skuList = newList

...
class InfluencerApixFragment : Fragment() {
    ...
    private fun callProductAPI() {
        ...
        viewModel.currentPageInfluencer = 1
        viewModel.clearInfluencerProductListData()
        val appPreferences = AppPreferences.getInstance(requireContext())
        val request = ApiService.InfluencerRequest(
            pageNo = "1",
            perPage = "4",
            search = searchValue,
            influencerId = appPreferences.getSellerId().toString(),
        )
        binding.progressBar.visibility = View.VISIBLE

        viewModel.getInfluencerProductList(appPreferences.getToken().toString(), request)
    }
    ...
    private fun setUpPagination() {
        ...
        scrollListener = ViewTreeObserver.OnScrollChangedListener {
            ...
            if (diff == 0) {
                if (viewModel.isLastPageInfluencer()) {
                    // No action for the last page
                } else {
                    viewModel.currentPageInfluencer++
                    viewModel.loadNextPageInfluencerProduct(
                        searchValue,
    ...
    private fun setUpRecyclerView() {
        productAdapter = SellerProductAdapter(
            ...
            val request = ApiService.CommissionRequest(
                id = AppPreferences.getInstance(requireContext()).getSellerId().toString(),
                percentage = value
            )
            viewModel.changeInfluencerCommission(AppPreferences.getInstance(requireContext()).getToken().toString(),request)
    ...
    private fun setViewModel() {
        ...
        viewModel.InfluencerData.observe(viewLifecycleOwner) { data ->
            ...
            try {
                ...
                if (!totalPages.isNullOrEmpty()) {
                    viewModel.totalPageCountInfluencer= totalPages.toInt()
                } else {
            ...
            if (viewModel.currentPageInfluencer == 1) {

...
class SellerListAdapter(
    private val onSkuSelected: (SellerListResponse) -> Unit
) : RecyclerView.Adapter<SellerListAdapter.SkuViewHolder>() {

    private var skuList: List<SellerListResponse> = emptyList()

    fun submitList(newList: List<SellerListResponse>) {
        skuList = newList
    ...
    inner class SkuViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(sku: SellerListResponse) {
